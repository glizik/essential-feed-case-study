...
//
//  Created by Lizik Gábor on 08/01/2024.
//
...
class EssentialFeedCacheIntegrationTests: XCTestCase {
    ...
    func test_loadImageData_deliversSavedDataOnASeparateInsance() {
    ...
    private func save(_ feed: [FeedImage], with loader: LocalFeedLoader, file: StaticString = #file, line: UInt = #line) {
        ...
        loader.save(feed) { result in
            if case let Result.failure(error) = result {
                XCTFail("Expected to save feed successully, got error: \(error)", file: file, line: line)
            }
            saveExp.fulfill()
        }
    ...
    private func expect(_ sut: LocalFeedLoader, toLoad expectedFeed: [FeedImage], file: StaticString = #file, line: UInt = #line) {
        ...
        sut.load { result in
            ...
            case let .success(loadedImage):
                XCTAssertEqual(loadedImage, expectedFeed, file: file, line: line)

            case let .failure(error):
                XCTFail("Expected susccessful feed result, got \(error) instead", file: file, line: line)
            }
