...
//  EssentialApp
//
//  Created by Lizik Gábor on 2025. 04. 17..
//
...
class CommentsUIIntegrationTests: XCTestCase {
    ...
    private func assertThat(_ sut: ListViewController, isRendering comments: [ImageComment], file: StaticString = #filePath, line: UInt = #line) {
        ...
        viewModel.comments.enumerated().forEach { index, comment in
            XCTAssertEqual(sut.commmentMessage(at: index), comment.message, "message at \(index)", file: file, line: line)
            XCTAssertEqual(sut.commmentDate(at: index), comment.date, "date at \(index)", file: file, line: line)
            XCTAssertEqual(sut.commmentUsername(at: index), comment.username, "username at \(index)", file: file, line: line)
        }
    }
